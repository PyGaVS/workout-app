...
export default function SetItem({ set }: PropsWithChildren<Props>){
    return (
      <div key={set.id} className='bg-bg/50 rounded-lg p-3 mt-5 border border-transparent relative'>
        {set.multiply > 1 && (
          <div className="px-2 text-center text-base font-bold text-surface rounded-full bg-accent absolute top-0 right-8 translate-y-[-0.9rem]">×{set.multiply}</div>
        )}
