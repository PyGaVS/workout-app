...
export default function StatCard({statLabel, stat, statBadge, statText, statDescription, skeleton} : StatCardType) {
    ...
    return (
        <div className="flex flex-col border-(--stats-border) border-1 rounded-xl py-4 px-6 w-1/4 h-50 justify-between">
            <div className="flex flex-col gap-2">
                <div className="flex justify-between">
                    <p className="text-(--text-muted)">{statLabel ?? 'Label'}</p>
                    {statBadge ?
                        <div className="border-1 border-(--stats-border) px-4 rounded-xl" style={{ boxShadow: `0 10px 15px -10px var(--text-muted)` }}>{statBadge}</div> : null}
                </div>
            ...
            <div className="flex flex-col gap-2">
                ...
                <p className="text-(--text-muted) text-sm">{statDescription ?? 'Pas ouf'}</p>
            </div>
        </div>
    )
}